...
export function ChallengesPanel({ supabase, canManage }: Props) {
  ...
  async function loadChallengeDetails(challengeId: string) {
    ...
    const [goalsResult, offersResult] = await Promise.all([
      supabase
        ...
        .select("id,challenge_id,target_km,public_label,display_order,is_active")
  ...
  async function createOffer(event: React.FormEvent) {
    event.preventDefault();
    if (!canManage || !selected) return;
    if (!offerStartsAt || !offerEndsAt || new Date(offerEndsAt) <= new Date(offerStartsAt)) {
      setMessage("Confira o início e o fim da oferta.");
      return;
    }
    const price = Number(offerPrice.replace(",", "."));
    const limit = Number(offerLimit);
    if (!Number.isFinite(price) || price < 0 || !Number.isInteger(limit) || limit < 1) {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        category_code: offerCategory.trim().toUpperCase().replace(/[^A-Z0-9_]/g, "_"),
    ...
    const { error: linkError } = await supabase.from("challenge_offer_goals").insert(links);
    if (linkError) {
  ...
  return (
    <section className="module-panel" aria-labelledby="challenges-title">
      ...
      <p className="section-description">
        Cadastre a edição e, em seguida, configure metas e ofertas de inscrição dentro dela.
      </p>
      ...
      {showForm ? (
        <form className="challenge-form" onSubmit={createChallenge}>
          ...
          <div className="form-actions"><button type="submit" disabled={busy}>{busy ? "Salvando…" : "Criar rascunho"}</button></div>
        </form>
      ) : null}
        ...
        {items.map((item) => (
          ...
          >
            <div className="challenge-main">
              ...
              <span>{formatDate(item.sports_starts_at)} a {formatDate(item.sports_ends_at)}</span>
            </div>
        ...
        <div className="challenge-detail">
          ...
          <div className="challenge-config-grid">
            <section className="config-card">
              <div className="section-heading">
                <div><h3>Metas</h3><p className="mini-description">Quilometragens disponíveis nesta edição.</p></div>
                {canManage ? <button type="button" className="compact" onClick={() => setShowGoalForm((value) => !value)} disabled={busy}>{showGoalForm ? "Cancelar" : "Adicionar meta"}</button> : null}
              </div>

              {showGoalForm ? (
                <form className="compact-form" onSubmit={createGoal}>
                  <div className="form-grid two">
                    <label>Quilômetros<input type="number" min={1} value={goalKm} onChange={(event) => setGoalKm(event.target.value)} placeholder="300" required /></label>
                    <label>Nome exibido<input value={goalLabel} onChange={(event) => setGoalLabel(event.target.value)} placeholder="300 km" /></label>
                  </div>
                  <div className="form-actions"><button type="submit" disabled={busy}>Salvar meta</button></div>
                </form>
              ) : null}

              <div className="simple-list">
                {goals.map((goal) => <div className="simple-row" key={goal.id}><strong>{goal.public_label || `${goal.target_km} km`}</strong><span>{goal.is_active ? "Ativa" : "Inativa"}</span></div>)}
            ...
            <section className="config-card">
              <div className="section-heading">
                <div><h3>Ofertas de inscrição</h3><p className="mini-description">Preço, prazo, limite e metas disponíveis.</p></div>
                {canManage ? <button type="button" className="compact" onClick={() => setShowOfferForm((value) => !value)} disabled={busy || !goals.length}>{showOfferForm ? "Cancelar" : "Adicionar oferta"}</button> : null}
              </div>

              {showOfferForm ? (
                <form className="compact-form" onSubmit={createOffer}>
                  <div className="form-grid two">
                    <label>Nome interno<input value={offerInternalName} onChange={(event) => setOfferInternalName(event.target.value)} placeholder="Normal agosto" required minLength={2} /></label>
                    <label>Nome para o público<input value={offerPublicName} onChange={(event) => setOfferPublicName(event.target.value)} placeholder="Inscrição Agosto" required minLength={2} /></label>
                  </div>
                  <div className="form-grid two">
                    <label>Categoria<input value={offerCategory} onChange={(event) => setOfferCategory(event.target.value)} placeholder="NORMAL" required /></label>
                    <label>Valor (R$)<input inputMode="decimal" value={offerPrice} onChange={(event) => setOfferPrice(event.target.value)} placeholder="44,90" required /></label>
                  </div>
                  <div className="form-grid two">
                    <label>Início das inscrições<input type="datetime-local" value={offerStartsAt} onChange={(event) => setOfferStartsAt(event.target.value)} required /></label>
                    <label>Fim das inscrições<input type="datetime-local" value={offerEndsAt} onChange={(event) => setOfferEndsAt(event.target.value)} required /></label>
                  </div>
                  <label>Limite por participante<input type="number" min={1} value={offerLimit} onChange={(event) => setOfferLimit(event.target.value)} required /></label>
                  <fieldset className="goal-picker">
                    ...
                    {goals.filter((goal) => goal.is_active).map((goal) => (
                      <label className="check-row" key={goal.id}>
                        ...
                        <span>{goal.public_label || `${goal.target_km} km`}</span>
                      </label>
                    ))}
                  </fieldset>
                  <div className="form-actions"><button type="submit" disabled={busy}>Salvar oferta em rascunho</button></div>
                </form>
              ) : null}
              ...
              <div className="simple-list">
                {offers.map((offer) => (
                  <div className="offer-row" key={offer.id}>
                    <div><strong>{offer.public_name}</strong><span>{offer.internal_name} · {offer.category_code}</span></div>
                    <div className="offer-side"><strong>{formatMoney(Number(offer.price))}</strong><span>{formatDate(offer.registration_starts_at)} a {formatDate(offer.registration_ends_at)}</span><span>{offerStatusLabel[offer.status]} · limite {offer.max_per_participant}</span></div>
                  </div>
                ))}
      ...
      {items.length ? <button type="button" className="link-button" onClick={() => void loadChallenges(selectedId)} disabled={busy}>Atualizar lista</button> : null}
    </section>
  );
}
