...
insert into public.app_permissions (code, description) values
  ('public_registrations.read', 'Consultar pré-inscrições públicas'),
  ('public_registrations.manage', 'Processar ou rejeitar pré-inscrições públicas')
...
where r.code = 'platform_admin'
  and p.code in ('public_registrations.read', 'public_registrations.manage')
on conflict do nothing;

drop policy if exists public_registration_requests_admin_read
...
for select
to authenticated
using (public.has_permission('public_registrations.read'));
...
as $$
begin
  if not public.has_permission('public_registrations.manage') then
    raise exception 'Permissão insuficiente para rejeitar a solicitação';
  end if;
...
begin
  ...
  select count(*), min(p.id)
    into participant_matches, found_participant_id
