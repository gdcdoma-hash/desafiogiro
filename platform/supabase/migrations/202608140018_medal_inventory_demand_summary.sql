...
select
  ...
  coalesce(regs.reserved_registration_count, 0)::bigint as reserved_registration_count,
  greatest(
    coalesce(regs.confirmed_count, 0) - coalesce(regs.reserved_registration_count, 0),
    0
  ...
  greatest(
    coalesce(regs.confirmed_count, 0) - coalesce(regs.reserved_registration_count, 0) - coalesce(stock.available_balance, 0),
    0
...
left join lateral (
  select
    ...
    ) as reserved_registration_count
